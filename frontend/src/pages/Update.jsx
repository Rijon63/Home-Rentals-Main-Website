...
const AMENITIES = [
  ...
  "Water Tank/Storage",
  "Temple/Proximity to Heritage Sites",
  "Pet-Friendly",
];
...
const Update = () => {
  ...
  useEffect(() => {
    const fetchProperty = async () => {
      try {
        ...
        if (response.data.success) {
          ...
          const latDMS = latitude ? decimalToDMS(latitude, true) : { degrees: "", minutes: "", seconds: "", direction: "N" };
          const lonDMS = longitude ? decimalToDMS(longitude, false) : { degrees: "", minutes: "", seconds: "", direction: "E" };
          setFormData({
            ...
            latitudeDegrees: latDMS.degrees.toString(),
            latitudeMinutes: latDMS.minutes.toString(),
            latitudeSeconds: latDMS.seconds.toString(),
            latitudeDirection: latDMS.direction,
            longitudeDegrees: lonDMS.degrees.toString(),
            longitudeMinutes: lonDMS.minutes.toString(),
            longitudeSeconds: lonDMS.seconds.toString(),
            longitudeDirection: lonDMS.direction,
            roadCondition: property.roadCondition || "",
            ...
            renovationYear: property.renovationYear?.toString() || "",
            propertyFacing: property.propertyFacing || "",
            landOwnershipType: property.landOwnershipType || "",
            ...
            gasConnection: property.gasConnection || "",
    ...
    fetchProperty();
  }, [id, navigate, authLoading, isLoggedIn]);
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "province") {
      ...
      setAvailableMunicipalities(MUNICIPALITIES[DISTRICTS[value]?.[0]] || []);
      setFormData((prev) => ({
        ...prev,
        district: DISTRICTS[value]?.[0] || "",
        municipality: MUNICIPALITIES[DISTRICTS[value]?.[0]]?.[0] || "",
      }));
    } else if (name === "district") {
      setAvailableMunicipalities(MUNICIPALITIES[value] || []);
      setFormData((prev) => ({
        ...prev,
        municipality: MUNICIPALITIES[value]?.[0] || "",
      }));
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formdata.append("ward", formData.ward);
