  ...
  return (
    ...
    >
      ...
      <div className="relative h-64">
        <img
          src={property.image && property.image[0] ? property.image[0] : "/placeholder-image.jpg"}
        ...
        <div className="absolute top-4 left-4 flex flex-col gap-2">
          ...
          <span className={`text-xs font-medium px-3 py-1.5 rounded-full shadow-md 
            ${property.availability?.toLowerCase().includes('rent') 
              ? 'bg-green-600 text-white' 
              : 'bg-purple-600 text-white'}`}>
            For {property.availability || "Buy"}
          </span>
        </div>
      ...
      <div className="p-6">
        ...
        <div className="flex items-center text-gray-600 mb-4">
          ...
          <span className="line-clamp-1">{property.location || "Unknown Location"}</span>
        </div>
        ...
        <div className="flex justify-between items-center py-3 border-y border-gray-100 mb-4">
          <div className="flex items-center gap-1">
            ...
            <span className="text-sm text-gray-600">{property.beds || 0} {property.beds !== 1 ? 'Beds' : 'Bed'}</span>
          </div>
          <div className="flex items-center gap-1">
            <Bath className="w-4 h-4 text-blue-500" />
            <span className="text-sm text-gray-600">{property.baths || 0} {property.baths !== 1 ? 'Baths' : 'Bath'}</span>
          </div>
          <div className="flex items-center gap-1">
            <Maximize className="w-4 h-4 text-blue-500" />
            <span className="text-sm text-gray-600">{property.sqft || 0} sqft</span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center text-blue-600 font-bold">
            ...
            <span className="text-xl">{Number(property.price || 0).toLocaleString('en-IN')}</span>
          </div>
          
          <div className="text-sm bg-blue-50 text-blue-700 px-2 py-1 rounded-md flex items-center">
            <Building className="w-3.5 h-3.5 mr-1" />
            {property.availability?.toLowerCase().includes('rent') ? 'Rental' : 'Purchase'}
          </div>
        </div>
      </div>
    </motion.div>
...
const PropertiesShow = () => {
      ...
      try {
        ...
        if (response.data.success && Array.isArray(response.data.properties)) {
          ...
          const processedProperties = featuredProperties.map(property => ({
            ...
            availability: property.availability || "Buy",
          ...
          throw new Error('Invalid API response: No properties found');
        }
      } catch (err) {
        console.error('Error fetching properties:', err);
        setError('Failed to load properties. Showing sample data instead.');
  ...
  const filteredProperties = activeCategory === 'all' 
    ...
    : properties.filter(property => 
        property.type && property.type.toLowerCase() === activeCategory.toLowerCase()
      );
