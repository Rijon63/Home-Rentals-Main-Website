...
const Update = () => {
    ...
    const fetchProperty = async () => {
      try {
        ...
        if (response.data.success) {
          ...
          const latitudeDMS = decimalToDMS(property.latitude || 27.7172, true);
          const longitudeDMS = decimalToDMS(property.longitude || 85.3240, false);
          setFormData({
            title: property.title || '',
            description: property.description || '',
            type: property.type || PROPERTY_TYPES[0],
            availability: property.availability || AVAILABILITY_TYPES[0],
            province: property.province || PROVINCES[0],
            district: property.district || (DISTRICTS[property.province] ? DISTRICTS[property.province][0] : ''),
            municipality: property.municipality || (property.district && MUNICIPALITIES[property.district] ? MUNICIPALITIES[property.district][0] : ''),
            wardNumber: property.wardNumber || '',
            streetAddress: property.streetAddress || '',
            nearbyLandmark: property.nearbyLandmark || '',
            roadCondition: property.roadCondition || ROAD_CONDITIONS[0],
            roadWidth: property.roadWidth || '',
            price: property.price || '',
            securityDeposit: property.securityDeposit || '',
            rentalPeriod: property.rentalPeriod || RENTAL_PERIODS[0],
            ...
            propertyFacing: property.propertyFacing || PROPERTY_FACING[0],
            landOwnershipType: property.landOwnershipType || LAND_OWNERSHIP_TYPES[0],
            amenities: property.amenities || [],
            electricityConnection: property.electricityConnection || false,
            waterSupply: property.waterSupply || WATER_SUPPLY_TYPES[0],
            sewageSystem: property.sewageSystem || SEWAGE_SYSTEMS[0],
            gasConnection: property.gasConnection || GAS_CONNECTIONS[0],
            phone: property.phone || '',
            latitudeDegrees: latitudeDMS.degrees.toString(),
            latitudeMinutes: latitudeDMS.minutes.toString(),
            latitudeSeconds: latitudeDMS.seconds.toString(),
            latitudeDirection: latitudeDMS.direction,
            longitudeDegrees: longitudeDMS.degrees.toString(),
            longitudeMinutes: longitudeDMS.minutes.toString(),
            longitudeSeconds: longitudeDMS.seconds.toString(),
            longitudeDirection: longitudeDMS.direction,
            images: [],
            rating: property.rating?.toString() || '0'
          });
          setPreviewUrls(property.image || []);
          setAvailableDistricts(DISTRICTS[property.province] || []);
          setAvailableMunicipalities(property.district && MUNICIPALITIES[property.district] ? MUNICIPALITIES[property.district] : []);
        } else {
  ...
  useEffect(() => {
    if (formData.province && DISTRICTS[formData.province]) {
      const districts = DISTRICTS[formData.province];
      ...
      setAvailableMunicipalities(formData.district && MUNICIPALITIES[formData.district] ? MUNICIPALITIES[formData.district] : []);
    } else {
  ...
  useEffect(() => {
    if (formData.district && MUNICIPALITIES[formData.district]) {
      const municipalities = MUNICIPALITIES[formData.district];
      setAvailableMunicipalities(municipalities);
      setFormData(prev => ({
        ...prev,
        municipality: municipalities.includes(formData.municipality) ? formData.municipality : municipalities[0]
      }));
    } else {
      setAvailableMunicipalities([]);
